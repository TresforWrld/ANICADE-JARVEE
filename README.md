...
| `play: track title` | Plays an uploaded or built-in track by name |
| `repeat this song` / `repeat playlist` / `repeat off` | Controls music looping |
| `list songs` | Reads the current playlist |
| `system check` | Reports speech, microphone, media, cache, and OCR status |
| `remember my exam is Friday` | Saves a local memory note |
| `what do you remember` | Reads recent memory notes |
| `list schedules` | Reads saved schedules |
| `what can you do` | Reads a quick command guide |
