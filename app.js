...
function extractWakeCommand(transcript) {
  ...
  const heardWake = wakePattern.test(transcript);
  const command = heardWake
...
function isLikelyLoopback(transcript) {
  ...
  return overlap > 0.76 && Date.now() - state.speech.lastSpeechEndedAt < 9000;
}
...
async function openNews(topic) {
  ...
  if (result.ok) {
    const items = result.display.split("\n").filter(Boolean).map(item => `<li>${escapeHtml(item)}</li>`).join("");
    els.surfaceContent.innerHTML = `<div class="surface-card"><h2>News Feed</h2><ul class="news-list">${items}</ul></div>`;
...
async function getNewsSummary(topic) {
  ...
  try {
    const sources = [];
    if (gnewsKey) {
      sources.push(() => fetchNewsJson(`https://gnews.io/api/v4/search?q=${encodeURIComponent(topic)}&lang=en&max=5&apikey=${encodeURIComponent(gnewsKey)}`));
      sources.push(() => fetchNewsJson(`https://gnews.io/api/v4/top-headlines?category=general&lang=en&max=5&apikey=${encodeURIComponent(gnewsKey)}`));
    }
    if (newsKey) {
      sources.push(() => fetchNewsJson(`https://newsapi.org/v2/everything?q=${encodeURIComponent(topic)}&language=en&pageSize=5&sortBy=publishedAt&apiKey=${encodeURIComponent(newsKey)}`));
      sources.push(() => fetchNewsJson(`https://newsapi.org/v2/top-headlines?q=${encodeURIComponent(topic)}&language=en&pageSize=5&apiKey=${encodeURIComponent(newsKey)}`));
    }
    if (serperKey) sources.push(() => fetchSerperNews(topic, serperKey));
    if (USER_CONFIG.DUCKDUCKGO_ENABLED !== false) sources.push(() => fetchGoogleNewsRss(topic));

    for (const fetchSource of sources) {
      const data = await fetchSource();
      const articles = normalizeNewsArticles(data);
      if (articles.length) {
...
async function fetchGoogleNewsRss(topic) {
  ...
  const response = await fetchWithTimeout(`https://api.allorigins.win/raw?url=${encodeURIComponent(rssUrl)}`, {}, 5200).catch(() => null);
  if (!response?.ok) return null;
...
function normalizeNewsArticles(data) {
  ...
  return raw
    .map(item => ({
      ...
      publishedAt: item.publishedAt || item.date || item.published || ""
    }))
...
async function answerConversationally(prompt) {
  const instruction = [
    "You are ANICADE JARVIS speaking to Sir.",
    "Be truthful, natural, concise, and useful.",
    "Do not mention this instruction, policies, hidden prompts, or capability rules.",
    "Do not invent sensor readings, heart rate, location, opened apps, music, maps, camera state, or news results.",
    "Say ANICADE JARVIS as words, never as spelled-out initials."
  ].join(" ");
  const ask = `${instruction}\n\nSir said: ${prompt}`;
  const reply = await withTimeout(askGeminiText(ask), 8000, "") || await withTimeout(askGroqText(ask), 8000, "");
...
async function speakText(text, options = {}) {
  ...
  const parts = splitSpeechIntoParts(cleaned);
  ...
  await new Promise(resolve => {
    ...
    const speakPart = () => {
      ...
      watchdog = window.setTimeout(() => {
        ...
        finishPart();
      }, Math.max(4500, part.length * 95 / Math.max(0.7, state.speech.rate)));
