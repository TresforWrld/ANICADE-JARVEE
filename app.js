...
const DEFAULT_MUSIC_FULL_VOLUME = 0.82;
const DEFAULT_MUSIC_DUCK_REDUCTION = 0.24;
const STARTUP_MUSIC_LYRIC_HINTS = [
  "should i stay",
  "should i go",
  "stay or should i go",
  "let me know",
...
const state = {
  ...
  music: {
    audio: new Audio(),
    volume: Number(localStorage.getItem("assistant_music_volume") || RAW_ASSISTANT_CONFIG.MUSIC_FULL_VOLUME || DEFAULT_MUSIC_FULL_VOLUME),
    restoreVolume: null,
    ducked: false,
    fadeFrame: 0,
...
async function initVoiceCore() {
  ...
  lockAssistantVoice();
  beginStartupShield();
  await withTimeout(startStartupMusic(), 2200, false);

  try {
    await speakText(buildStartupGreeting(), { preferElevenLabs: false });
  } finally {
    endStartupShield();
    startAudioAnalysis();
...
function beginStartupShield() {
  state.startup.active = true;
  const shieldMs = Number(USER_CONFIG.startupShieldMs || RAW_ASSISTANT_CONFIG.STARTUP_SHIELD_MS || 48000);
  state.startup.shieldUntil = Date.now() + shieldMs;
  state.speech.ignoreTranscriptsUntil = state.startup.shieldUntil;
  state.speech.keepListening = false;
  pauseRecognitionForSpeech();
}

function endStartupShield() {
  state.startup.active = false;
  const tailMs = Number(USER_CONFIG.startupTranscriptCooldownMs || RAW_ASSISTANT_CONFIG.STARTUP_TRANSCRIPT_COOLDOWN_MS || 10000);
  state.startup.shieldUntil = Date.now() + tailMs;
  state.speech.ignoreTranscriptsUntil = Math.max(state.speech.ignoreTranscriptsUntil, state.startup.shieldUntil);
...
function initSpeechRecognition() {
  ...
  recognition.onerror = event => {
    ...
    if (code === "aborted" || state.speech.pausedForSpeech || state.startup.active) {
      if (state.speech.keepListening && !state.speech.pausedForSpeech && !state.startup.active) {
    ...
    if (state.speech.keepListening && !state.speech.pausedForSpeech) {
      scheduleListenRestart();
    }
  };
...
function handleTranscript(rawTranscript) {
  ...
  const now = Date.now();
  if (isLikelyMusicTranscript(transcript) || isLikelyLoopback(transcript)) return;
  if (state.startup.active || now < state.startup.shieldUntil) {
    if (!heardWake) return;
  }
  if (state.speech.speaking || now < state.speech.ignoreTranscriptsUntil) {
    if (!heardWake && now < state.speech.ignoreTranscriptsUntil) return;
  }
  ...
  state.commandChain = state.commandChain
    ...
    .catch(() => reportCommandIssue());
}

function reportCommandIssue(message) {
  setStatus(message || "That did not complete. Try rephrasing or a narrower request.", 5200);
}
...
function isLikelyMusicTranscript(transcript) {
  const heard = normalizeSpeech(transcript);
  if (!heard) return false;
  const musicPlaying = Boolean(state.music.audio && !state.music.audio.paused && !state.music.audio.muted);
  const inStartupWindow = state.startup.active || Date.now() < state.startup.shieldUntil;
  if (!musicPlaying && !inStartupWindow) return false;
  if (STARTUP_MUSIC_LYRIC_HINTS.some(phrase => heard.includes(phrase))) return true;
  if (/\b(verse|chorus|guitar|drums|la la|na na|oh oh|yeah yeah)\b/.test(heard)) return true;
  const titleHints = normalizeSpeech(STARTUP_SONG_QUERY);
  if (titleHints && heard.includes(titleHints)) return true;
  return false;
}
...
function isLikelyLoopback(transcript) {
  ...
  if (!heard || !spoken) return isLikelyMusicTranscript(transcript);
  ...
  if (heard.includes("systems are fully operational") || heard.includes("virtual artificial intelligence")) return true;
  const words = heard.split(" ").filter(Boolean);
  if (words.length < 4) return isLikelyMusicTranscript(transcript);
  ...
  if ((overlap > 0.72 || similarity > 0.68) && Date.now() - state.speech.lastSpeechEndedAt < 12000) return true;
  return isLikelyMusicTranscript(transcript);
}
...
async function openBrowserSurface(queryOrUrl) {
  ...
  setStatus("Structured crawler sweep initialized inside the app.", 2500);
...
function buildNewsLoadingHtml(topic) {
  return `<div class="surface-card news-card"><div class="news-kicker">GOOGLE GROUNDED STREAM</div><h2>${escapeHtml(topic)}</h2><p>Fetching current technology and AI briefs from available live providers.</p><div class="news-loading"><span></span><span></span><span></span></div></div>`;
}
...
function renderNewsFeed(topic, articles, streaming) {
  ...
  return `<div class="surface-card news-card"><div class="news-kicker">${streaming ? "GROUNDING" : "GOOGLE GROUNDED STREAM"}</div><h2>${escapeHtml(topic)}</h2><ul class="news-list">${items}</ul></div>`;
}
...
async function speakErrorOnce(message) {
  ...
  const now = Date.now();
  if (!clean || (clean === state.lastSpokenError && now - state.lastSpokenErrorAt < 25000)) return;
  state.lastSpokenError = clean;
  state.lastSpokenErrorAt = now;
  reportCommandIssue(clean);
}
...
async function answerConversationally(prompt) {
  ...
  const instruction = [
    ...
    "You may split complex work into internal specialist sub-agents, but never claim external execution unless a tool actually ran.",
  ...
  const fallback = "I can help with that, but I need a little more detail before I act.";
  await speakText(reply || fallback);
}
...
async function playMusicTrack(track, options = {}) {
  ...
  state.music.audio.volume = state.music.ducked
    ? computeDuckedMusicVolume(state.music.restoreVolume ?? targetVolume)
    : targetVolume;
...
function duckMusicForSpeech() {
  ...
  fadeMusicVolume(computeDuckedMusicVolume(audio.volume), 140);
}
...
function restoreMusicAfterSpeech() {
  ...
  fadeMusicVolume(clampAudioVolume(restored || getMusicFullVolume(), getMusicFullVolume()), 520);
  state.music.restoreVolume = null;
  state.music.ducked = false;
}

function computeDuckedMusicVolume(currentVolume) {
  const current = clampAudioVolume(currentVolume, getMusicFullVolume());
  const reduction = Math.max(0.2, Number(USER_CONFIG.musicDuckReduction || RAW_ASSISTANT_CONFIG.MUSIC_DUCK_REDUCTION || DEFAULT_MUSIC_DUCK_REDUCTION));
  const floor = clampAudioVolume(Number(USER_CONFIG.musicDuckFloor || RAW_ASSISTANT_CONFIG.MUSIC_DUCK_FLOOR || 0.14), 0.14);
  return Math.max(floor, current * (1 - reduction));
}

function getMusicFullVolume() {
  return clampAudioVolume(Number(USER_CONFIG.musicFullVolume || RAW_ASSISTANT_CONFIG.MUSIC_FULL_VOLUME || DEFAULT_MUSIC_FULL_VOLUME), DEFAULT_MUSIC_FULL_VOLUME);
}

function getMusicDuckVolume() {
  return computeDuckedMusicVolume(getMusicFullVolume());
}
...
async function setMusicVolume(params = {}) {
  ...
  if (state.music.ducked) {
    ...
    state.music.audio.volume = computeDuckedMusicVolume(state.music.audio.volume);
  } else {
...
function cleanTextForSpeech(text) {
  ...
  return withoutMeta
    .replace(/\b(request failed|network error|fetch failed|api error|http error|status code \d{3})\b[^.!?]*[.!?]?/gi, "A related feature is temporarily unavailable. ")
    .replace(/\b(that\s+)?service\s+(isn['’]?t|is not|aint|ain['’]?t)\s+working\b[.!]*/gi, "A related feature is temporarily unavailable.")
    .replace(/\b(api key|apikey|access token|bearer token)\b[^.!?]*/gi, "")
    .replace(/\b(provider|providers)\s+(failed|unavailable|down)\b[.!]*/gi, "A related feature is temporarily unavailable.")
...
function applyPronunciationHints(text) {
  return String(text || "")
    .replace(/\bJARVIS\b/gi, "Jarvis")
    .replace(/\bANICADE\b/gi, "Anicade")
    .replace(/\bATLAS\b/gi, "Atlas")
    .replace(/\bSCHEMA\b/gi, "Schema")
    .replace(/\bECHO\b/gi, "Echo")
    .replace(/\bFORGE\b/gi, "Forge")
    .replace(/\bPULSE\b/gi, "Pulse")
    .replace(/\bVISION\b/gi, "Vision")
    .replace(/\bZambia\b/gi, "ZAM-bee-ah")
    .replace(/\bKabwe\b/gi, "KAH-bway")
...
async function speakText(text, options = {}) {
  ...
  const parts = splitSpeechIntoParts(spokenText, 160);
  ...
  await new Promise(resolve => {
    ...
    const speakPart = () => {
      ...
      const finishPart = () => {
        ...
        index += 1;
        window.setTimeout(speakPart, 42);
      };
      utterance.onend = () => {
        finishPart();
      };
...
function getTranscriptCooldownMs(text = "") {
  const base = Number(USER_CONFIG.selfSpeechCooldownMs || RAW_ASSISTANT_CONFIG.SELF_SPEECH_COOLDOWN_MS || 3200);
  const musicPlaying = Boolean(state.music.audio && !state.music.audio.paused && !state.music.audio.muted);
  const spokenLength = String(text || "").length;
  return base + (musicPlaying ? 1400 : 0) + Math.min(2400, spokenLength * 14);
}
...
function beginAssistantSpeech(text) {
  ...
  const cooldownMs = getTranscriptCooldownMs(text);
  state.speech.ignoreTranscriptsUntil = Date.now() + Math.max(cooldownMs, Math.min(7200, text.length * 32));
...
function endAssistantSpeech(speechId = state.speech.speechId) {
  ...
  const cooldownMs = getTranscriptCooldownMs(state.speech.lastSpoken);
  state.speech.ignoreTranscriptsUntil = Date.now() + cooldownMs;
...
function updateNumericReadouts() {
  ...
  setText(els.bodyTemp, `${t.bodyTemp.toFixed(1)} C`);
  setText(els.oxygenSat, `${Math.round(t.oxygen)}%`);
  setText(els.respRate, `${t.respiration.toFixed(1)}/MIN`);
  setText(els.skinConductance, `${t.conductance.toFixed(1)} uS`);
  setText(els.neuralLoad, `${Math.round(t.neuralLoad)}%`);
  setText(els.alphaWave, `${t.alpha.toFixed(1)} HZ`);
  setText(els.betaWave, `${t.beta.toFixed(1)} HZ`);
  setText(els.thetaWave, `${t.theta.toFixed(1)} HZ`);
  setText(els.coherence, `${Math.round(t.coherence)}%`);
  setText(els.bandwidthVal, `${t.bandwidth.toFixed(2)} GB/S`);
  setText(els.packetLoss, `${t.packetLoss.toFixed(4)}%`);
  setText(els.coreTemp, `${t.coreTemp.toFixed(1)} C`);
  setText(els.entropyVal, t.entropy.toFixed(3));
