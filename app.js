...
function checkVoiceWakeup(cmdLower) {
  ...
  const wakeupFuzzy = [
    "jarvis", "javis", "jarves", "charvis", "java", "friday", "freeday", "fryday", "phriday", "anicade", "anycade", "anicat", "arcade", "wake up"
  ];
...
function playMusic() {
  if (!state.audioPlayer) {
    state.audioPlayer = new Audio(state.musicPlaylist[state.currentMusicIndex]);
    state.audioPlayer.loop = true;
    state.audioPlayer.volume = 0.25; // low background volume
  }
  state.audioPlayer.play()
    .then(() => {
      speakText("Playing background tracks, Sir.");
      addLog('system', 'Background music active.');
    })
    .catch(err => {
...
function nextMusicTrack() {
  if (state.audioPlayer) {
    ...
    state.audioPlayer.src = state.musicPlaylist[state.currentMusicIndex];
    state.audioPlayer.play()
      .then(() => {
        speakText("Playing the next track, Sir.");
        addLog('system', `Switched to track ${state.currentMusicIndex + 1}`);
      })
      .catch(err => {
        console.error(err);
      });
...
function rebuildLogsUI() {
  ...
  state.conversationLogs.forEach(log => {
    ...
    msgEl.className = 'caption-message';
    if (log.sender === 'user') {
      msgEl.innerHTML = `<span class="caption-user">[You]:</span> ${log.text}`;
    } else if (log.sender === 'ai') {
      msgEl.innerHTML = `<span class="caption-ai">[AI]:</span> ${log.text}`;
    } else {
      msgEl.innerHTML = `<span class="caption-system">[System]:</span> ${log.text}`;
    }
    elements.captionArea.appendChild(msgEl);
  });
  elements.captionArea.scrollTop = elements.captionArea.scrollHeight;
