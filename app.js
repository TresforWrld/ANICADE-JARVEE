...
function speakText(text) {
  ...
  if (!state.speechSynthesis || state.voiceMuted) return;
...
function getMapEmbedUrl() {
  ...
  if (view.routeFrom && view.routeTo) {
    return `https://www.google.com/maps?saddr=${encodeURIComponent(view.routeFrom)}&daddr=${encodeURIComponent(view.routeTo)}&z=${view.zoom || 13}&output=embed`;
  }
  const layerQuery = view.mode === 'oil' ? `oil pipelines ${view.query}`
    : view.mode === 'swamps' ? `swamps wetlands ${view.query}`
    : view.mode === 'roads' ? `roads ${view.query}`
    : view.query;
  const type = view.mode === 'satellite' ? '&t=k' : '';
  return `https://www.google.com/maps?q=${encodeURIComponent(layerQuery)}&z=${view.zoom || 14}${type}&output=embed`;
}
...
function openMapsSurface(cmd = '') {
  ...
  if (routeMatch) {
    ...
    refreshMapSurface(`Route plotted, ${addressUser()}.`);
    return;
  }
  const cleaned = text
    ...
    .replace(/\b(satellite|oil lines?|roads?|swamps?|wetlands?|zoom in|zoom out)\b/ig, '')
    .trim();
  state.mapView.query = cleaned || state.mapView.query || state.homeLocation.label;
...
function changeMapZoom(delta) {
  ...
  refreshMapSurface(`Map zoom ${delta > 0 ? 'increased' : 'reduced'}, ${addressUser()}.`);
}
...
function processVoiceCommand(cmd) {
  ...
  const openUrlMatch = cmd.match(/\b(?:open|show|load)\s+(https?:\/\/\S+|(?:www\.)?[a-z0-9.-]+\.[a-z]{2,}(?:\/\S*)?)/i);
  if (openUrlMatch) {
...
async function welcomeGreeting() {
  ...
  let text = pickVariant([
    `Good ${timeOfDay}, ${addressUser()}. Systems are online.`,
    `Good ${timeOfDay}, ${addressUser()}. Visual, voice, and command cores are standing by.`,
    `Good ${timeOfDay}, ${addressUser()}. The interface is ready for your next instruction.`,
    `Good ${timeOfDay}, ${addressUser()}. Operational status is stable.`
  ]);
  ...
  if (!briefedToday && hour >= 5 && hour < 12) {
    ...
    text = pickVariant([
      `Welcome back, ${addressUser()}.`,
      `Back online with you, ${addressUser()}.`,
