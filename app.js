...
function processVoiceCommand(cmd) {
  ...
  if (lowerCmd.startsWith("search ") || lowerCmd.includes("search the internet") || lowerCmd.includes("google ")) {
    const query = cmd
...
async function searchInternet(query) {
  ...
  try {
    ...
    if (!answer) {
      answer = `I could not find a clean instant result, Sir. Opening a web search for ${query}.`;
    ...
    updateStatusVisuals();
    window.open(`https://www.google.com/search?q=${encodeURIComponent(query)}`, '_blank');
    speakText(`I opened a live web search for ${query}, Sir.`);
  }
}
