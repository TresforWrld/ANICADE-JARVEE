...
function handleTranscript(rawTranscript) {
  ...
  if (new RegExp(`^\\s*(hey|hi|okay|ok)\\s+${escapeRegExp(wakeWord)}\\b`, "i").test(transcript)) return;
  const command = new RegExp(`^\\s*${escapeRegExp(wakeWord)}\\b`, "i").test(transcript)
    ? transcript.replace(new RegExp(`^\\s*${escapeRegExp(wakeWord)}\\b[,.\\s]*`, "i"), "").trim()
    : transcript;

  state.commandChain = state.commandChain
    .then(() => runCommand(command || "system check"))
    .catch(() => speakText("Unable to complete that action, Sir."));
}
...
async function interpretCommand(command) {
  ...
  const aiIntent = await interpretWithAi(command);
...
async function interpretWithAi(command) {
  const allowed = [
    ...
    "open_schematics", "close_module", "open_browser", "open_news", "open_video", "play_music",
  ...
  if (gemini) return normalizeIntent(gemini);
  const groq = await askGroqForJson(prompt);
  if (groq) return normalizeIntent(groq);
  return null;
}
...
function parseLocalIntent(command) {
  ...
  if (/\bpause|stop music\b/.test(lower) && /\b(music|song|track|audio)\b/.test(lower)) return { action: "pause_music", params: {}, confidence: 0.95 };
  if (/\bnext\b/.test(lower) && /\b(music|song|track|audio)\b/.test(lower)) return { action: "next_track", params: {}, confidence: 0.95 };
  if (/\b(play|music|song|track|audio|open music|music widget)\b/.test(lower)) return { action: "play_music", params: {}, confidence: 0.88 };
...
async function calculateRoute(params) {
  ...
  await speakText(`Route view is open, Sir. Estimated direct distance is ${estimate.distanceKm} kilometres. Live turn by turn routing requires a routing API key, so I will not pretend to have calculated traffic-aware directions.`);
}
...
function mapModeResponse(mode) {
  const responses = {
    ...
    traffic: "Traffic layer requested, Sir. Availability depends on the embedded map provider.",
    pipeline: "Pipeline overlay requested, Sir. I can display the tactical layer, but verified live pipeline data is not connected.",
    swamps: "Wetland layer requested, Sir. Verified environmental data is not connected yet.",
...
async function openBrowserSurface(queryOrUrl) {
  ...
  await speakText(`Browser surface opened, Sir. If the site blocks embedding, I will not claim it loaded successfully.`);
}

async function openNews(topic) {
  const clean = String(topic || "Zambia latest news").trim();
  const html = `<div class="surface-card"><h2>News Feed</h2><p>Live news request prepared for ${escapeHtml(clean)}. Verified article retrieval needs network access from the browser and may depend on News API quota.</p></div>`;
  showSurface("PULSE", clean.toUpperCase(), { html });
  const summary = await getNewsSummary(clean);
  await speakText(summary);
}

async function getNewsSummary(topic) {
  const key = USER_CONFIG.GNEWS_API_KEY || USER_CONFIG.NEWSAPI_KEY;
  if (!key) return `Unable to retrieve live news at the moment, Sir. No news API key is configured.`;
  try {
    const url = USER_CONFIG.GNEWS_API_KEY
      ? `https://gnews.io/api/v4/search?q=${encodeURIComponent(topic)}&lang=en&max=3&apikey=${encodeURIComponent(USER_CONFIG.GNEWS_API_KEY)}`
      : `https://newsapi.org/v2/everything?q=${encodeURIComponent(topic)}&pageSize=3&apiKey=${encodeURIComponent(USER_CONFIG.NEWSAPI_KEY)}`;
    const response = await fetch(url);
    if (!response.ok) return `Unable to retrieve live news at the moment, Sir. The news service returned ${response.status}.`;
    const data = await response.json();
    const articles = data.articles || [];
    if (!articles.length) return `I could not find verified live headlines for ${topic}, Sir.`;
    const headlines = articles.slice(0, 3).map(article => article.title).filter(Boolean).join(". ");
    return `Here are the latest verified headlines I found for ${topic}, Sir. ${headlines}.`;
  } catch {
    return `Unable to retrieve live news at the moment, Sir. Network access is not available from this session.`;
  }
}
