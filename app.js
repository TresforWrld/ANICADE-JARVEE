...
function processVoiceCommand(cmd) {
  ...
  if (playTitle && !/music|song|tune$/i.test(playTitle[1].trim())) {
...
function extractSearchQuery(cmd) {
  return cmd
    .replace(/search the internet for/i, '')
    .replace(/search web for/i, '')
    .replace(/search for/i, '')
    .replace(/look up/i, '')
    .replace(/google/i, '')
    .replace(/^(the )?internet for/i, '')
    .replace(/^search/i, '')
    .trim();
}
...
async function searchInternet(query) {
  ...
  try {
    ...
    if (!answer) {
      answer = `I could not find a clean instant result, Sir. I opened a live web search for ${query}. Please check your internet connection if the page does not load.`;
    ...
    updateStatusVisuals();
    window.open(`https://www.google.com/search?q=${encodeURIComponent(query)}`, '_blank');
    speakText(`I could not reach the search service, Sir. Please connect to the internet, then say try again. I opened a Google search tab as a fallback.`);
  }
}
...
async function runLocalPollinations(customText) {
  ...
  try {
    ...
    state.isAnalyzing = false;
    updateStatusVisuals();
    speakText("I could not reach the online answer service, Sir. Please connect to the internet, then say try again.");
  }
}
...
async function runLocalOCRWithText(ocrText, actionType, customText) {
  try {
    ...
    state.isAnalyzing = false;
    updateStatusVisuals();
    speakText("I could not complete that screen analysis, Sir. Please check your internet connection, or upload a clearer image and say read screen again.");
...
function handleMusicUpload(event) {
  ...
  state.musicPlaylist.push({ title, src });
  state.currentMusicIndex = state.musicPlaylist.length - 1;
  addLog('system', `Song uploaded: ${title}. Say "play ${title}" to start it.`);
  playMusic();
}
