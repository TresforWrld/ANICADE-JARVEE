const RAW_ASSISTANT_CONFIG = (() => {
  try {
    return window.ASSISTANT_CONFIG || window.JARVIS_CONFIG || {};
  } catch {
    return {};
  }
})();
...
const state = {
  ...
  music: {
    audio: new Audio(),
    volume: Number(localStorage.getItem("assistant_music_volume") || localStorage.getItem("jarvis_music_volume") || 0.72),
    restoreVolume: null,
    ducked: false,
    fadeFrame: 0,
...
function handleTranscript(rawTranscript) {
  ...
  if (state.sleepMode) {
    if (/\b(wake|resume|activate|start listening|come back|back online)\b/i.test(command || transcript) || heardWake) {
...
function buildClarifyingIntent(command) {
  const question = "What would you like me to do with that?";
...
function setStatus(message, timeoutMs = 4500) {
  state.status.message = String(message || "");
