...
const STARTUP_INTRODUCTION = "Allow me to introduce myself, I am JARVIS, a virtual artificial intelligence running in this browser interface. I can help with voice commands, search, news, maps, and conversation when providers are configured. Dashboard vitals are simulated visuals, not medical sensors. Running system check.";
...
const DEFAULT_NEWS_REFRESH_MS = 2 * 60 * 1000;
...
const state = {
  ...
  surfaceLoadTimer: 0,
  sleepMode: false,
  idleTimeoutMs: DEFAULT_IDLE_SLEEP_MS,
  activity: {
    lastUserAt: Date.now(),
    idleTimer: 0
  },
...
function init() {
  state.music.audio.volume = clampAudioVolume(state.music.volume, getMusicFullVolume());
...
function installActivityTracking() {
  ...
  window.clearInterval(state.activity.idleTimer);
  state.activity.idleTimer = window.setInterval(checkAutoSleep, 1000);
}
...
function checkAutoSleep() {
  if (state.sleepMode || state.speech.speaking) return;
  const timeoutMs = DEFAULT_IDLE_SLEEP_MS;
  if (Date.now() - state.activity.lastUserAt >= timeoutMs) {
    enterSleepMode({ silent: true, reason: "idle" });
  }
}
...
function scheduleListenRestart() {
  ...
  state.speech.restartTimer = window.setTimeout(startContinuousListening, Math.max(cooldown, state.speech.speaking ? 420 : 60));
}
...
function handleTranscript(rawTranscript) {
  ...
  if (!command) {
    if (heardWake) speakText("Yes?", { shortCooldown: true });
    return;
  }
...
async function interpretCommand(command) {
  ...
  if (localIntent.action === "clarify" && localIntent.confidence >= 0.9) return localIntent;

  const aiIntent = await withTimeout(interpretWithAi(command), 1200, null);
  if (aiIntent) return aiIntent;

  if (localIntent.action === "clarify") {
    return { action: "general_chat", params: { prompt: command }, confidence: 0.5 };
  }
  return localIntent.confidence > 0 ? localIntent : { action: "general_chat", params: { prompt: command }, confidence: 0.35 };
}
...
async function interpretWithAi(command) {
  const allowed = [
    ...
    "sleep", "wake", "hide_visual_core", "show_visual_core", "recent_commands", "daily_briefing", "calendar_status", "weather_status", "reminders_status", "tasks_status", "general_chat", "clarify"
  ];
...
function parseLocalIntent(command) {
  ...
  if (/\b(daily briefing|morning briefing|evening briefing|status briefing|brief me on my day)\b/.test(lower)) return { action: "daily_briefing", params: {}, confidence: 0.94 };
  ...
  if (/\b(weather|forecast|temperature|rain|humidity)\b/.test(lower)) return { action: "weather_status", params: { query: command }, confidence: 0.92 };
  if (/\b(reminder|reminders|alarm|alarms)\b/.test(lower)) return { action: "reminders_status", params: {}, confidence: 0.92 };
  if (/\b(task|tasks|todo|to-?do|checklist)\b/.test(lower)) return { action: "tasks_status", params: {}, confidence: 0.94 };
  ...
  if (/\b(close|hide|dismiss|remove|get rid of|shut).*\b(visual core|neural core|brain|core|particles|particle field)\b/.test(lower)) return { action: "hide_visual_core", params: {}, confidence: 0.94 };
  if (/\b(show|restore|open|bring back|display).*\b(visual core|neural core|brain|core|particles|particle field)\b/.test(lower)) return { action: "show_visual_core", params: {}, confidence: 0.94 };
  ...
  if (/\b(news|headline|latest|reporter|feed)\b/.test(lower)) return { action: "open_news", params: { topic: extractTarget(command, /^(open|show|give|read|latest|news|headlines|feed|about|on|\s)+/ig) || "technology and AI developments" }, confidence: 0.88 };
  ...
  if (shouldClarifyCommand(lower)) return buildClarifyingIntent(command);
...
function shouldClarifyCommand(lower) {
  const text = String(lower || "").trim();
  if (!text) return true;
  if (!looksLikeCommand(text)) return false;
  if (/\?$/.test(text) || /\b(who|what|when|where|why|how|tell me|explain|answer)\b/.test(text)) return false;
...
async function executeIntent(intent, rawCommand) {
  ...
  if (action === "daily_briefing") return speakDailyBriefing();
  ...
  if (action === "weather_status") return speakWeatherStatus(params.query || rawCommand);
  if (action === "reminders_status") return speakText("No reminder service is connected in this build. None found.");
  if (action === "tasks_status") return speakText("No task list is connected in this build. No records available.");
  ...
  if (action === "open_news") return openNews(params.topic || params.query || "technology and AI developments");
...
async function openBrowserSurface(queryOrUrl) {
  ...
  const target = buildBrowserTarget(clean || "technology and AI developments");
  if (!target.ok) {
  ...
  setStatus("Opening in-app browser view.", 2500);
  await speakText("Browser surface opened for the requested node.");
}
...
function buildBrowserTarget(queryOrUrl) {
  ...
  if (/^https?:\/\//i.test(clean)) {
    try {
      ...
      if (protocol !== "http:" && protocol !== "https:") return { ok: false };
  ...
  const query = clean
    .replace(/^(node|destination|dest|search)\s*[:=-]\s*/i, "")
    .trim() || "technology and AI developments";
  return {
    ok: true,
...
async function openNews(topic) {
  const clean = String(topic || "technology and AI developments").trim();
  stopNewsRefresh();
...
function scheduleNewsRefresh(topic) {
  ...
  state.news.refreshTimer = window.setTimeout(() => refreshNewsFeed(topic), Number(USER_CONFIG.newsRefreshMs || DEFAULT_NEWS_REFRESH_MS));
}
...
async function getNewsSummary(topic, onUpdate = null) {
  ...
  try {
    ...
    if (serperKey) sources.push({ name: "Google Search Grounding", run: () => fetchSerperNews(topic, serperKey) });
    sources.push({ name: "Google News Grounding", run: () => fetchGoogleNewsRss(topic) });
    ...
    if (articles.length) {
      ...
      return {
        ok: true,
        articles,
        spoken: `Here are the latest grounded briefs I found for ${topic}. ${headlines.join(". ")}.`
      };
    }
    return { ok: false, message: `I could not find verified briefs for ${topic} yet. The feed is staying open and retrying live sources shortly.` };
  } catch {
...
function buildNewsLoadingHtml(topic) {
  return `<div class="surface-card news-card"><div class="news-kicker">NEWS FEED</div><h2>${escapeHtml(topic)}</h2><p>Fetching headlines from configured providers when available.</p><div class="news-loading"><span></span><span></span><span></span></div></div>`;
}
...
function renderNewsFeed(topic, articles, streaming) {
  ...
  return `<div class="surface-card news-card"><div class="news-kicker">${streaming ? "LOADING" : "NEWS FEED"}</div><h2>${escapeHtml(topic)}</h2><ul class="news-list">${items}</ul></div>`;
}
...
function looksLikeResearchQuery(prompt) {
  const lower = String(prompt || "").toLowerCase().trim();
  if (!lower) return false;
  if (/\b(open|show|play|pause|close|map|camera|music|sleep|wake|standby)\b/.test(lower) && !/\b(search|look up|research|news|latest|current)\b/.test(lower)) return false;
  return /\b(what|who|when|where|why|how|latest|current|today|recent|news|research|look up|search|find out|explain|compare|price|weather)\b/.test(lower)
    || /\?$/.test(lower);
}

async function fetchResearchSnippets(query) {
  const clean = String(query || "").trim();
  if (!clean) return "";
  const collected = [];
  const serperKey = USER_CONFIG.SERPER_API_KEY;
  const braveKey = USER_CONFIG.BRAVE_SEARCH_API_KEY;
  const ddgEnabled = USER_CONFIG.DUCKDUCKGO_ENABLED !== false;

  const tasks = [];
  if (serperKey) tasks.push(fetchSerperWebSnippets(clean, serperKey));
  if (braveKey) tasks.push(fetchBraveWebSnippets(clean, braveKey));
  if (ddgEnabled) tasks.push(fetchDuckDuckGoSnippets(clean));

  const results = await Promise.allSettled(tasks);
  for (const result of results) {
    if (result.status === "fulfilled" && result.value) collected.push(...result.value);
  }
  const unique = [];
  const seen = new Set();
  for (const item of collected) {
    const key = normalizeSpeech(item).slice(0, 120);
...
async function fetchSerperWebSnippets(query, key) {
  ...
  return (data.organic || []).slice(0, 4).map(item => {
    ...
    return title ? `${title}${snippet ? ` — ${snippet}` : ""}` : snippet;
  }).filter(Boolean);
}

async function fetchBraveWebSnippets(query, key) {
  ...
  if (!response?.ok) return [];
  const data = await response.json();
  return (data.web?.results || []).slice(0, 4).map(item => {
    const title = String(item.title || "").trim();
    const desc = String(item.description || "").trim();
    return title ? `${title}${desc ? ` — ${desc}` : ""}` : desc;
  }).filter(Boolean);
}
...
async function fetchDuckDuckGoSnippets(query) {
  ...
  const response = await fetchWithTimeout(url, {}, 4200).catch(() => null);
  if (!response?.ok) return [];
  const data = await response.json();
  const lines = [];
  if (data.AbstractText) lines.push(`${data.Heading || "Summary"}: ${data.AbstractText}`);
...
function extractWeatherPlace(command) {
  const text = String(command || "");
  const match = text.match(/\b(?:in|at|for)\s+([a-z][a-z\s,.-]{2,40})/i);
  if (match) return match[1].replace(/[?.!]+$/g, "").trim();
  return "";
}

async function speakDailyBriefing() {
  ...
  try {
    ...
    parts.push(`Local time in ${zone} is ${formatted}.`);
  } catch {
    parts.push("Local time is unavailable for the configured time zone.");
  }
  parts.push("No calendar connected. No scheduled meetings.");
  parts.push("No reminder service connected. None found.");
  parts.push("No task list connected. No records available.");
  const recent = state.commandHistory.slice(-4).map(item => item.command).filter(Boolean);
  if (recent.length) parts.push(`Recent session commands: ${recent.join("; ")}.`);
  else parts.push("No recent commands in this session yet.");
  const followUps = state.followUp.memory.slice(-2).map(item => item.question).filter(Boolean);
  if (followUps.length) parts.push(`Saved follow-up prompts: ${followUps.join("; ")}.`);
  const hasNews = USER_CONFIG.GNEWS_API_KEY || USER_CONFIG.NEWSAPI_KEY || USER_CONFIG.SERPER_API_KEY;
  if (hasNews) parts.push("Say open news with a topic for live headlines from configured providers.");
  else parts.push("No news API keys configured.");
  const hasWeather = USER_CONFIG.OPENWEATHER_API_KEY || USER_CONFIG.WEATHERAPI_KEY;
  if (hasWeather) parts.push("Ask for weather in a place for a live forecast.");
  else parts.push("No weather provider connected.");
  return speakText(parts.join(" "));
}

async function speakWeatherStatus(command) {
  const owKey = USER_CONFIG.OPENWEATHER_API_KEY;
  const waKey = USER_CONFIG.WEATHERAPI_KEY;
  if (!owKey && !waKey) return speakText("No weather provider is connected in this build.");
  const place = extractWeatherPlace(command)
    || state.map.home?.city
    || state.map.home?.label
    || state.map.query
    || "Kabwe";
  if (owKey) {
    const spoken = await fetchOpenWeatherSummary(place, owKey);
    if (spoken) return speakText(spoken);
  }
  if (waKey) {
    const spoken = await fetchWeatherApiSummary(place, waKey);
    if (spoken) return speakText(spoken);
  }
  return speakText(`No verified weather records available for ${place}.`);
}

async function fetchOpenWeatherSummary(place, key) {
  const url = `https://api.openweathermap.org/data/2.5/weather?q=${encodeURIComponent(place)}&units=metric&appid=${encodeURIComponent(key)}`;
  const response = await fetchWithTimeout(url, {}, 5000).catch(() => null);
  if (!response?.ok) return "";
  const data = await response.json();
  ...
  if (!Number.isFinite(temp) || !desc) return "";
  const feelsPart = Number.isFinite(feels) ? `, feels like ${Math.round(feels)} degrees` : "";
  return `OpenWeather reports ${city} at ${Math.round(temp)} degrees Celsius${feelsPart}, ${desc}.`;
}

async function fetchWeatherApiSummary(place, key) {
  const url = `https://api.weatherapi.com/v1/current.json?key=${encodeURIComponent(key)}&q=${encodeURIComponent(place)}`;
  const response = await fetchWithTimeout(url, {}, 5000).catch(() => null);
  if (!response?.ok) return "";
  const data = await response.json();
  const temp = data.current?.temp_c;
  const desc = data.current?.condition?.text;
  const city = data.location?.name || place;
  if (!Number.isFinite(temp) || !desc) return "";
  return `WeatherAPI reports ${city} at ${Math.round(temp)} degrees Celsius, ${desc}.`;
}

async function answerConversationally(prompt) {
  const context = buildConversationContext();
  let researchBlock = "";
  if (looksLikeResearchQuery(prompt)) {
    const snippets = await fetchResearchSnippets(prompt);
    researchBlock = snippets
      ? `\n\nGrounded search snippets (use only for current facts; do not invent beyond them):\n${snippets}`
      : "\n\nNo live search snippets were retrieved. If the user asks for current facts, say plainly that no verified records are available.";
  }
  const instruction = [
    `You are JARVIS, a helpful, professional virtual artificial intelligence${USER_CONFIG.userName ? ` for ${USER_CONFIG.userName}` : ""}.`,
    "You operate as a web-based voice-first assistant in this interface.",
    ...
    "Orbit labels on the neural core animation are visual only; never claim separate autonomous agents executed work unless a module actually opened.",
    ...
    "End with one brief follow-up recommendation question that keeps the human in the loop.",
    ...
    "Never mention fictional backstories, branded film-inspired details, wealthy inventor personas, or imaginary household upgrade projects.",
...
async function askFastAssistantText(prompt) {
  ...
  if (!requests.length) {
    return "I can still help right now. Ask your question directly and I will answer from built-in knowledge or guide the next action I can perform here.";
  }
  ...
  }))).catch(() => "I can still help with that. External AI providers are temporarily unavailable, so ask in plain language and I will answer with available context.");
}
...
function renderConversation() {
  ...
  if (!log) return;
  const entries = state.conversation.entries.slice(-6);
  if (!entries.length) {
    log.textContent = "";
    return;
  }
  log.innerHTML = entries.map(entry => {
    ...
    return `<article class="conversation-entry conversation-${escapeHtml(entry.role)}"><b>${escapeHtml(role)}</b><span>${escapeHtml(entry.text)}</span></article>`;
  }).join("");
...
function getMemoryStorageKey(suffix) {
  const userKey = String(USER_CONFIG.userName || USER_CONFIG.USER_NAME || "default")
    .trim()
    .toLowerCase()
    .replace(/[^a-z0-9._-]+/g, "_")
    .slice(0, 48) || "default";
  return `assistant_${suffix}_${userKey}`;
}

function loadFollowUpMemory() {
  try {
    const saved = JSON.parse(localStorage.getItem(getMemoryStorageKey("followup_memory")) || "[]");
...
function saveFollowUpMemory() {
  try {
    localStorage.setItem(getMemoryStorageKey("followup_memory"), JSON.stringify(state.followUp.memory.slice(-12)));
  } catch {}
}
...
function isRemoteMusicUrl(src) {
  return /^https?:\/\//i.test(String(src || ""));
}

async function musicSourceAvailable(src) {
  if (!src) return false;
  if (isRemoteMusicUrl(src)) return true;
  try {
...
async function startStartupMusic() {
  ...
  state.music.index = 0;
  let track = state.music.tracks[0];
  if (track?.src && !isRemoteMusicUrl(track.src) && !(await musicSourceAvailable(track.src))) {
    track = { ...track, src: "" };
    state.music.tracks[0] = track;
  }
  if (!track?.src) {
    return false;
  }
...
async function playMusicTrack(track, options = {}) {
  ...
  if (!isRemoteMusicUrl(track.src) && !(await musicSourceAvailable(track.src))) {
    throw new Error("missing music source");
  }
  if (state.music.audio.src !== track.src) state.music.audio.src = track.src;
  const targetVolume = options.fullVolume ? getMusicFullVolume() : clampAudioVolume(state.music.volume, getMusicFullVolume());
  if (options.fullVolume) {
    state.music.volume = getMusicFullVolume();
    localStorage.setItem("assistant_music_volume", String(state.music.volume));
  }
  const shouldDuck = state.music.ducked || state.speech.speaking;
  if (shouldDuck && !state.music.ducked) {
    state.music.ducked = true;
    state.music.restoreVolume = targetVolume;
  }
  state.music.audio.volume = shouldDuck
    ? getMusicDuckVolume()
    : targetVolume;
...
function duckMusicForSpeech() {
  ...
  if (!audio || audio.muted || state.music.ducked) return;
  state.music.restoreVolume = audio.paused ? clampAudioVolume(state.music.volume, getMusicFullVolume()) : audio.volume;
  state.music.ducked = true;
  if (audio.paused) {
    audio.volume = getMusicDuckVolume();
    return;
  }
  fadeMusicVolume(getMusicDuckVolume(), 120);
}
...
function restoreMusicAfterSpeech() {
  ...
  fadeMusicVolume(clampAudioVolume(restored || getMusicFullVolume(), getMusicFullVolume()), 500);
  state.music.restoreVolume = null;
  state.music.ducked = false;
}

function getMusicFullVolume() {
  return clampAudioVolume(Number(USER_CONFIG.musicFullVolume || RAW_ASSISTANT_CONFIG.MUSIC_FULL_VOLUME || 0.55), 0.55);
}

function getMusicDuckVolume() {
  return clampAudioVolume(Number(USER_CONFIG.musicDuckVolume || RAW_ASSISTANT_CONFIG.MUSIC_DUCK_VOLUME || 0.08), 0.08);
}

function clampAudioVolume(value, fallback = 0.55) {
...
async function setMusicVolume(params = {}) {
  ...
  if (state.music.ducked) {
    ...
    state.music.audio.volume = getMusicDuckVolume();
  } else {
...
async function exitSleepMode(options = {}) {
  ...
  if (options.fromClap) playClashRiff();
  startContinuousListening();
  if (options.silent) return;
  ...
  try {
    const response = await fetch("https://api.groq.com/openai/v1/chat/completions", {
      ...
      body: JSON.stringify({
        model: USER_CONFIG.groqModel || "llama-3.1-8b-instant",
        messages: [
          { role: "system", content: "You are JARVIS, a professional web-based virtual AI assistant. Answer exactly what the user asks, be concise, warm, and capable, do not add fluff, do not use canned apologies, be truthful, do not claim unverified actions, avoid fictional movie or household upgrade references, and end with one brief follow-up recommendation question." },
          { role: "user", content: prompt }
        ],
        temperature: 0.55,
        max_tokens: 420
...
function buildSystemReport() {
  ...
  const providers = [
    USER_CONFIG.GEMINI_API_KEY || USER_CONFIG.geminiApiKey ? "Gemini" : "",
    USER_CONFIG.GROQ_API_KEY || USER_CONFIG.groqApiKey ? "Groq" : "",
    USER_CONFIG.SERPER_API_KEY ? "search" : ""
  ].filter(Boolean);
  const providerLine = providers.length ? `Configured providers: ${providers.join(", ")}.` : "No external AI keys are configured in config.js.";
  return `System check complete. ${mic}. ${recognition}. ${providerLine} Simulated dashboard fuel metrics show uplink ${t.bandwidth.toFixed(2)} gigabytes per second display, packet loss ${t.packetLoss.toFixed(4)} percent display, and core thermal ${t.coreTemp.toFixed(1)} degrees display.`;
}
...
function stripMoviePersonaText(text) {
  const blocked = [
    ...
    /\blaser\s+(controller|controllers|control|controls|array|arrays)\b/gi,
...
function splitSpeechIntoParts(text, maxLength = 220) {
  const clauses = String(text || "").match(/[^.!?;:]+[.!?;:]?/g) || [text];
  const parts = [];
  let current = "";
...
function speechCooldownMs(text, options = {}) {
  if (options.shortCooldown) return Number(USER_CONFIG.shortSpeechCooldownMs || 380);
  const configured = Number(USER_CONFIG.selfSpeechCooldownMs || RAW_ASSISTANT_CONFIG.SELF_SPEECH_COOLDOWN_MS);
  const base = Number.isFinite(configured) && configured > 0 ? configured : 1100;
  const len = String(text || "").length;
  return Math.min(base, Math.max(420, Math.round(len * 14)));
}

function speechIgnoreMs(text, options = {}) {
  if (options.shortCooldown) return speechCooldownMs(text, options);
  const len = String(text || "").length;
  return Math.max(700, Math.min(2800, Math.round(len * 16)));
}

async function tryElevenLabs(text, timeoutMs = 1400) {
  if (!USER_CONFIG.elevenLabsApiKey || !USER_CONFIG.elevenLabsVoiceId) return false;
  const controller = new AbortController();
  const timeout = window.setTimeout(() => controller.abort(), timeoutMs);
  const modelId = USER_CONFIG.ELEVENLABS_MODEL || USER_CONFIG.elevenLabsModel || "eleven_turbo_v2_5";
...
async function speakText(text, options = {}) {
  ...
  const speechId = beginAssistantSpeech(cleaned, options);

  try {
    const useElevenLabs = options.preferElevenLabs === true
      || USER_CONFIG.preferElevenLabs === true
      || USER_CONFIG.USE_ELEVENLABS === true;
    if (useElevenLabs && await tryElevenLabs(cleaned, Number(USER_CONFIG.ttsNetworkTimeoutMs || 1400))) {
      endAssistantSpeech(speechId, options);
      return;
    }
  } catch {
    state.speech.currentAudio = null;
  ...
  const chunkMax = Number(USER_CONFIG.speechChunkMaxChars || RAW_ASSISTANT_CONFIG.SPEECH_CHUNK_MAX_CHARS || 360);
  const parts = splitSpeechIntoParts(spokenText, chunkMax);
...
function beginAssistantSpeech(text, options = {}) {
  ...
  state.speech.ignoreTranscriptsUntil = Date.now() + speechIgnoreMs(text, options);
...
function endAssistantSpeech(speechId = state.speech.speechId, options = {}) {
  if (speechId !== state.speech.speechId) return;
  const resumeDelay = speechCooldownMs(state.speech.lastSpoken, options);
  state.speech.speaking = false;
  state.speech.lastSpeechEndedAt = Date.now();
  state.speech.ignoreTranscriptsUntil = Date.now() + resumeDelay;
...
function readAudioSnapshot(seconds) {
  if (state.audio.analyser && state.audio.freqData && state.audio.timeData) {
    ...
    for (let i = 1; i < state.audio.freqData.length; i += 1) {
      ...
      const hz = i * state.audio.sampleRate / state.audio.analyser.fftSize;
      if (hz >= 900 && hz <= 5200) {
        highBandPeak = Math.max(highBandPeak, state.audio.freqData[i]);
        highBandTotal += state.audio.freqData[i];
        highBandCount += 1;
      }
    }
    ...
    const highBandAverage = highBandCount ? highBandTotal / highBandCount : 0;
    const energy = Math.min(1, rms * 8 + max / 510);
    detectClapWake({ energy, dominantHz, peak: max / 255, highPeak: highBandPeak / 255, highAverage: highBandAverage / 255 });
...
function detectClapWake(snapshot) {
  const now = Date.now();
  const { energy, dominantHz, peak, highPeak, highAverage } = snapshot;
  const lastEnergy = state.clap.lastEnergy;
  const lastPeak = state.clap.lastPeak;
  state.clap.lastEnergy = energy;
  state.clap.lastPeak = peak;
  if (now < state.clap.cooldownUntil) return;
  const broadbandSnap = highPeak > 0.56 && highAverage > 0.08;
  const risingEdge = energy > 0.52 && peak > 0.62 && lastEnergy < 0.34 && lastPeak < 0.38 && dominantHz > 850 && broadbandSnap;
  if (!risingEdge) return;
  const gap = now - state.clap.lastClapAt;
  if (gap > 900) state.clap.count = 0;
  state.clap.lastClapAt = now;
  state.clap.count += 1;
  if (state.clap.count >= 2 && gap >= 90 && gap <= 900) {
    state.clap.count = 0;
    state.clap.cooldownUntil = now + 1600;
    recordUserActivity();
    if (state.sleepMode) {
      exitSleepMode({ fromClap: true, silent: true });
    } else if (!state.speech.speaking) {
      playClashRiff();
    }
  }
}
...
function updateNumericReadouts() {
  const t = state.telemetry;
  setText(els.spectrumPeak, `${t.spectrumPeak.toFixed(1)} DB`);
  setText(els.waveRms, `${t.waveRms.toFixed(3)} RMS`);
  setText(els.dominantHz, `${Math.round(t.dominantHz)} HZ`);
  setText(els.heartRate, `${Math.round(t.heartRate)} SIM`);
  setText(els.bodyTemp, `${t.bodyTemp.toFixed(1)} C SIM`);
  setText(els.oxygenSat, `${Math.round(t.oxygen)}% SIM`);
  setText(els.respRate, `${t.respiration.toFixed(1)}/MIN SIM`);
  setText(els.skinConductance, `${t.conductance.toFixed(1)} uS SIM`);
  setText(els.neuralLoad, `${Math.round(t.neuralLoad)}% SIM`);
  setText(els.alphaWave, `${t.alpha.toFixed(1)} HZ SIM`);
  setText(els.betaWave, `${t.beta.toFixed(1)} HZ SIM`);
  setText(els.thetaWave, `${t.theta.toFixed(1)} HZ SIM`);
  setText(els.coherence, `${Math.round(t.coherence)}% SIM`);
  setText(els.bandwidthVal, `${t.bandwidth.toFixed(2)} GB/S SIM`);
  setText(els.packetLoss, `${t.packetLoss.toFixed(4)}% SIM`);
  setText(els.coreTemp, `${t.coreTemp.toFixed(1)} C SIM`);
  setText(els.entropyVal, `${t.entropy.toFixed(3)} SIM`);
  setText(els.streamHex, Array.from({ length: 24 }, () => Math.floor(Math.random() * 255).toString(16).padStart(2, "0").toUpperCase()).join(" "));
}
...
function playClashRiff() {
  ...
  shaper.curve = makeDriveCurve(280);
  ...
  notes.forEach((freq, index) => {
    ...
    osc.frequency.exponentialRampToValueAtTime(freq * 0.995, start + 0.11);
...
function makeDriveCurve(amount = 200) {
  const samples = 256;
...
// Visual-only orbit labels for the neural core; not separate running agents.
function spawnBrainAgents(command) {
  const spawnId = ++state.brain.spawnId;
  const fragments = partitionCommandForAgents(command);
  state.brain.agents = fragments.map((label, index) => ({
    id: `${spawnId}-${index}`,
    label,
    status: "processing",
    angle: -Math.PI / 2 + index * (Math.PI * 2 / Math.max(1, fragments.length)),
...
function partitionCommandForAgents(command) {
  const text = String(command || "").trim();
  if (!text) return ["intent"];
  const lower = text.toLowerCase();
  const parts = [];
  if (/\b(search|look up|news|latest|browser|web|research|find)\b/.test(lower)) parts.push("grounding");
  if (/\b(create|make|build|generate|write|code|design|schematic|diagram)\b/.test(lower)) parts.push("synthesis");
  if (/\b(map|route|open|play|camera|music|video|show|launch)\b/.test(lower)) parts.push("execution");
  if (/\b(explain|summarize|answer|what|why|how|who|when|where)\b/.test(lower)) parts.push("reasoning");
  if (!parts.length) parts.push("intent", "reasoning");
  return [...new Set(parts)].slice(0, 3);
}

function completeBrainAgents(spawnId) {
  state.brain.agents.forEach((agent, index) => {
    if (!agent.id.startsWith(`${spawnId}-`)) return;
    window.setTimeout(() => {
      agent.status = "completed";
      agent.completedAt = performance.now();
    }, 150 + index * 140);
  });
  window.setTimeout(() => {
    state.brain.agents = state.brain.agents.filter(agent => !agent.id.startsWith(`${spawnId}-`));
  }, 5200);
}
...
function initBrainCanvas() {
  ...
  function draw(now = performance.now()) {
    ...
    const speaking = state.speech.speaking ? 1 : 0;
    ctx.clearRect(0, 0, w, h);

    const core = ctx.createRadialGradient(cx, cy, 0, cx, cy, r * 0.5);
    core.addColorStop(0, `rgba(223, 231, 234, ${0.72 + energy * 0.22})`);
    core.addColorStop(0.18, `rgba(0, 218, 243, ${0.54 + energy * 0.24})`);
    core.addColorStop(0.48, "rgba(199, 231, 109, 0.16)");
    core.addColorStop(1, "rgba(0, 218, 243, 0)");
    ctx.fillStyle = core;
    ctx.beginPath();
    ctx.arc(cx, cy, r * (0.18 + energy * 0.035 + speaking * 0.018), 0, Math.PI * 2);
    ctx.fill();

    for (let ring = 0; ring < 6; ring += 1) {
      const ringRadius = r * (0.28 + ring * 0.1);
      ctx.save();
      ctx.translate(cx, cy);
      ctx.rotate(now * (0.00018 + ring * 0.00008) * (ring % 2 ? -1 : 1));
      ctx.strokeStyle = ring % 2
        ? `rgba(199, 231, 109, ${0.13 + energy * 0.12})`
        : `rgba(0, 218, 243, ${0.16 + energy * 0.16})`;
      ctx.lineWidth = 1 + ring * 0.12;
      ctx.setLineDash([Math.max(8, ringRadius * 0.11), Math.max(10, ringRadius * 0.07)]);
      ctx.beginPath();
      ctx.ellipse(0, 0, ringRadius, ringRadius * (0.54 + ring * 0.055), ring * 0.62, 0, Math.PI * 2);
      ctx.stroke();
      ctx.restore();
    }
    ctx.setLineDash([]);
    ...
    for (let i = 0; i < paths; i += 1) {
      const a = now * 0.0007 + i * Math.PI * 2 / paths;
      const inner = r * (0.2 + (i % 3) * 0.04);
      const outer = r * (0.68 + Math.sin(now * 0.001 + i) * 0.04);
      ctx.strokeStyle = `rgba(0, 218, 243, ${0.08 + energy * 0.12})`;
      ctx.lineWidth = 0.8;
      ctx.beginPath();
      ctx.moveTo(cx + Math.cos(a) * inner, cy + Math.sin(a) * inner);
      ctx.quadraticCurveTo(
        cx + Math.cos(a + 0.7) * r * 0.46,
        cy + Math.sin(a - 0.35) * r * 0.32,
        cx + Math.cos(a + 1.25) * outer,
        cy + Math.sin(a + 1.25) * outer
      );
      ctx.stroke();
    }

    state.brain.agents.forEach(agent => {
      agent.angle += 0.0038 + energy * 0.004;
      agent.pulse += 0.08;
      const orbit = r * agent.radius;
      const x = cx + Math.cos(agent.angle) * orbit;
      const y = cy + Math.sin(agent.angle) * orbit * 0.72;
      const completed = agent.status === "completed";
      const alpha = completed ? 0.54 : 0.86;
      ctx.strokeStyle = completed ? "rgba(199, 231, 109, 0.42)" : "rgba(255, 183, 120, 0.46)";
      ctx.lineWidth = completed ? 0.9 : 1.4;
      ctx.beginPath();
      ctx.moveTo(cx, cy);
      ctx.lineTo(x, y);
      ctx.stroke();
      ctx.fillStyle = completed ? `rgba(199, 231, 109, ${alpha})` : `rgba(255, 183, 120, ${alpha})`;
      ctx.beginPath();
      ctx.arc(x, y, 4.5 + Math.sin(agent.pulse) * 1.4 + (completed ? 0 : energy * 3), 0, Math.PI * 2);
      ctx.fill();
      ctx.strokeStyle = completed ? "rgba(199, 231, 109, 0.25)" : "rgba(255, 183, 120, 0.32)";
      ctx.beginPath();
      ctx.arc(x, y, 12 + Math.sin(agent.pulse) * 2.4, 0, Math.PI * 2);
      ctx.stroke();
    });
