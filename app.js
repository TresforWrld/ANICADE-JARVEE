...
const USER_CONFIG = {
  ...
  geminiApiKey: (window.JARVIS_CONFIG && window.JARVIS_CONFIG.geminiApiKey) || '',
  claudeApiKey: (window.JARVIS_CONFIG && window.JARVIS_CONFIG.claudeApiKey) || '',
  googleClientId: (window.JARVIS_CONFIG && window.JARVIS_CONFIG.googleClientId) || '',
  facebookAppId: (window.JARVIS_CONFIG && window.JARVIS_CONFIG.facebookAppId) || '',
  jsonbinId: '',
  jsonbinKey: '',
  jsonbinAccessKey: ''
};
...
const state = {
  ...
  selectedVoiceName: '',
  selectedLanguage: USER_CONFIG.language || 'en-GB',
  wakeWord: USER_CONFIG.wakeWord || 'jarvis',
...
function initVoiceSettings() {
  ...
  function loadVoices() {
    ...
    state.selectedVoiceName = englishVoice ? englishVoice.name : (voices[0] ? voices[0].name : '');
  }
...
function processVoiceCommand(cmd) {
  ...
  if (commandHasAny(intentCmd, ["show controls", "emergency controls"])) {
    showPanel('controls', { persist: true });
    speakText("Emergency controls revealed, Sir.");
    return;
  }
...
function updateStatusVisuals() {
  ...
  if (isAnal) {
    ...
    elements.voiceSubstatus.textContent = "";
  }
