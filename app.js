...
function closeVisualCore() {
  stopActiveStream();
  hideContextPanels();
  speakText(`Visual core closed, ${addressUser()}.`);
}
...
function openInstructionManual() {
  showPanel('help', { persist: true });
  speakText("Manual opened, Sir.");
}
...
function openWebSurface(url, label = 'Web View') {
  ...
  openSurfacePanel({ title: label, subtitle: safeUrl, url: safeUrl });
  speakText(`Opening ${label}, Sir.`);
}
  ...
  try {
    switch(action) {
      case 'open_tab':
        if (p.tab) {
          showPanel(p.tab);
          speakText(`${p.tab} panel opened, Sir.`);
        }
        break;
      case 'close_tab':
        hideContextPanels();
      ...
      case 'close_module':
        if (p.module === 'web_view') {
          closeSurfacePanel();
          speakText("Web view closed, Sir.");
        } else if (p.module === 'visual_core') {
          closeVisualCore();
        } else if (p.module === 'map') {
          closeSurfacePanel();
          speakText("Tactical map closed, Sir.");
        }
        break;
      case 'scroll':
...
async function processVoiceCommand(cmd) {
  ...
  if (commandHasAny(intentCmd, ["show conversation", "show captions", "show logs"])) {
    ...
    speakText("Captions are off, Sir. I opened the clean answer output instead.");
    state.isAnalyzing = false;
    updateStatusVisuals();
    return;
  }
  if (commandHasAny(intentCmd, ["show music player", "show playlist", "list songs", "show music"])) {
    showPanel('music');
    speakText("Music panel opened, Sir.");
    state.isAnalyzing = false;
    updateStatusVisuals();
    return;
  }
...
function updateStandbyClock() {
  ...
  if (elements.standbyClockMode) {
    elements.standbyClockMode.textContent = state.isStandby ? 'Standby' : state.isListening ? 'Listening' : 'Ready';
  }
}
