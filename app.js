...
async function runLocalPollinations(customText) {
  ...
  try {
    ...
    state.isAnalyzing = false;
    updateStatusVisuals();
    speakText("I can handle the local controls now, Sir. For a live source-backed answer, say internet scan followed by the topic.");
  }
}
...
function getLocalKnowledgeAnswer(promptText) {
  ...
  if (whatIsMatch) {
    return `I can give a source-backed answer for ${whatIsMatch[2]} if you say: internet scan ${whatIsMatch[2]}.`;
  }
  return "";
}
...
async function runLocalOCRWithText(ocrText, actionType, customText) {
  try {
    ...
    state.isAnalyzing = false;
    updateStatusVisuals();
    speakText("I could not complete that screen analysis, Sir. Please upload a clearer image, share a different window, or say internet scan followed by the topic.");
