...
const STARTUP_INTRODUCTION = "Allow me to introduce myself, I am JARVIS, a virtual artificial intelligence and I'm here to assist you with a variety of tasks as best I can, 2 hours a day, 7 days a week, importing all preferences from home interface. Systems are fully operational.";
const STARTUP_SONG_TITLE = "Should I Stay or Should I Go - The Clash";
    ...
    tracks: [
      {
        ...
        src: USER_CONFIG.startupSongUrl || USER_CONFIG.STARTUP_SONG_URL || USER_CONFIG.CLASH_TRACK_URL || "",
...
function scheduleIdleSleep() {
  window.clearTimeout(state.activity.idleTimer);
  state.activity.idleTimer = window.setTimeout(handleIdleSleep, Math.max(1000, state.idleTimeoutMs));
}

function handleIdleSleep() {
  const idleFor = Date.now() - state.activity.lastUserAt;
  if (state.sleepMode) return;
  if (state.speech.speaking || idleFor < state.idleTimeoutMs) {
    scheduleIdleSleep();
    return;
  }
  enterSleepMode({ silent: true });
}
...
function handleTranscript(rawTranscript) {
  ...
  state.commandChain = state.commandChain
    ...
    .then(result => {
      if (result === "timeout") return speakErrorOnce("That command took too long, Sir. I stopped waiting for it.");
      return null;
    })
...
async function openNews(topic) {
  ...
  }), 9500, { ok: false, message: "News is taking too long to respond, Sir." });
  if (result.ok) {
    els.surfaceContent.innerHTML = renderNewsFeed(clean, result.articles, false);
    await speakText(result.spoken);
  } else {
    els.surfaceContent.innerHTML = `<div class="surface-card news-card"><h2>Live News</h2><p>${escapeHtml(result.message || "News is unavailable right now, Sir.")}</p></div>`;
...
function startStartupMusic() {
  if (state.music.startupAttempted && !state.music.startupPending) return;
  ...
  playMusicTrack(track, { rememberVolume: false })
    .then(() => {
...
async function playMusicTrack(track, options = {}) {
  ...
  state.music.audio.volume = state.music.ducked
    ? Math.max(0.08, (state.music.restoreVolume ?? state.music.volume) * 0.22)
    : state.music.volume;
...
function duckMusicForSpeech() {
  ...
  audio.volume = Math.max(0.08, state.music.restoreVolume * 0.22);
}
...
function restoreMusicAfterSpeech() {
  ...
  audio.volume = Math.max(0, Math.min(1, restored));
  ...
  try {
    const response = await fetch("https://api.groq.com/openai/v1/chat/completions", {
      ...
      body: JSON.stringify({
        model: USER_CONFIG.groqModel || "llama-3.1-8b-instant",
        messages: [
          { role: "system", content: "You are ANICADE JARVIS, an AI assistant. Answer exactly what Sir asks, be concise, do not add fluff, be truthful, do not claim unverified actions, and ask a follow-up recommendation question only when necessary." },
          { role: "user", content: prompt }
        ],
        temperature: 0.55,
        max_tokens: 420
...
async function speakText(text, options = {}) {
  ...
  const parts = splitSpeechIntoParts(cleaned, 150);
  ...
  await new Promise(resolve => {
    ...
    const speakPart = () => {
      ...
      const finishPart = () => {
        ...
        index += 1;
        window.setTimeout(speakPart, 60);
      };
      utterance.onend = () => {
        finishPart();
      };
...
function readAudioSnapshot(seconds) {
  if (state.audio.analyser && state.audio.freqData && state.audio.timeData) {
    ...
    detectClapWake(energy, dominantHz);
...
function detectClapWake(energy, dominantHz) {
  const now = Date.now();
  if (!state.sleepMode || state.speech.speaking || now < state.clap.cooldownUntil) return;
  const risingEdge = energy > 0.64 && state.clap.lastEnergy < 0.28 && dominantHz > 900;
  state.clap.lastEnergy = energy;
  if (!risingEdge) return;
  if (now - state.clap.lastClapAt > 900) state.clap.count = 0;
  state.clap.lastClapAt = now;
  state.clap.count += 1;
  if (state.clap.count >= 2) {
    state.clap.count = 0;
    state.clap.cooldownUntil = now + 1800;
    recordUserActivity();
    exitSleepMode();
  }
}
